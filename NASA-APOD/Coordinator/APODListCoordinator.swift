...
@MainActor
class APODListCoordinator: Coordinator {
    ...
    func start() {
        ...
        let viewModel = APODListViewModel()
        
        viewModel.onSelectAPOD = { [weak self] apod in
            self?.showDetail(for: apod)
        }
        
        viewModel.onSelectFeaturedAPOD = { [weak self] apod in
            self?.showDetail(for: apod)
        }
        
        let viewController = APODListViewController(viewModel: viewModel)
